...
import { ThemedButton } from '@/app/_components/themed-button'
...
export function DataTable<TData, TValue> ( {
	...
	secondaryActionIcon }: DataTableProps<TData, TValue> )
{
	...
	return (
		...
		<CardContent className='flex flex-col lg:flex-row gap-5 justify-between'>
				<div className='lg:w-1/2 flex flex-col lg:flex-row gap-5'>
					...
					{ primaryActionIcon && primaryActionTitle && (
					<ThemedButton>{ primaryActionTitle } {primaryActionIcon}</ThemedButton>) }
				</div>
				{ secondaryActionTitle && secondaryActionIcon && (
				<div className='w-full lg:w-2/3 justify-end flex flex-col lg:flex-row gap-5'>
				<ThemedButton>{ secondaryActionTitle } {secondaryActionIcon}</ThemedButton>
				</div>)}
		</CardContent>
			<CardContent>
			<Table>
          ...
          {table.getRowModel().rows?.length ? (
            table.getRowModel().rows.map((row) => (
					<TableRow
						className='even:bg-slate-50 rounded-lg border-none'
                key={row.id}
